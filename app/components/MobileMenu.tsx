        ...
        >
            ...
            <div className="h-full overflow-y-auto">
                <div className="min-h-full flex flex-col justify-center px-8 py-24">
                    {/* 左對齊的導航選單 */}
                    <nav className="flex flex-col space-y-6 w-full max-w-sm mx-auto">
                        {menuItems.length > 0 ? (
                            menuItems.map((item, index) => {
                                ...
                                return (
                                    <div
                                        key={item.id}
                                        className={`transform transition-all duration-700 delay-[${index * 50}ms] ${isOpen ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
                                            }`}
                                    >
                                        {hasChildren ? (
                                            <button
                                                onClick={() => toggleExpand(item.id)}
                                                className="flex items-center gap-3 text-2xl font-light text-[var(--brand-ink)] hover:text-[var(--brand-gray)] tracking-wide transition-colors duration-300 w-full text-left"
                                            >
                                            ...
                                            <Link
                                                ...
                                                onClick={onClose}
                                                className="block text-2xl font-light text-[var(--brand-ink)] hover:text-[var(--brand-gray)] tracking-wide transition-colors duration-300"
                                            >
                                                {item.label}
                                            </Link>
                                        )}
                                        ...
                                        {hasChildren && (
                                            ...
                                            >
                                                <div className="space-y-2 pl-4 border-l border-[var(--brand-line)]">
                                                    {item.children.map((child) => (
                                                        <Link
                                                            key={child.id}
                                                            href={toInternalPath(child.url, child.path)}
                                                            onClick={onClose}
                                                            className="block text-base text-[var(--brand-gray)] hover:text-[var(--brand-ink)] tracking-wide transition-colors duration-300 py-1"
                                                        >
                                                            {child.label}
                                                        </Link>
                                                    ))}
                    ...
                    {/* Social Links */}
                    <div className={`mt-16 transition-opacity duration-700 delay-500 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
                        <div className="max-w-sm mx-auto">
                            <p className="text-xs text-[var(--brand-gray)] tracking-widest uppercase mb-4 font-sans">
                                Follow Us
                            </p>
                            <div className="flex gap-6">
                                {socialLinks.map((link) => (
                                    <a
                                        ...
                                        className="text-[var(--brand-gray)] hover:text-[var(--brand-ink)] transition-colors duration-300"
